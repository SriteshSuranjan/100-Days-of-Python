# 🐍 100 Days of Python

A **100 Days of Python** learning journey where I am learning Python through **daily practice, coding, exercises, and hands-on implementation**.

This repository contains the code and practice work from **Day 1 to Day 100**.

> **Learn every day. Code every day. Improve every day. 🚀**

---

## 🎯 Goals

* Build a strong foundation in Python
* Understand Python programming concepts
* Practice coding consistently
* Strengthen problem-solving skills
* Implement concepts through code
* Complete exercises and challenges
* Develop a consistent daily coding habit
* Build a foundation for advanced Python and future projects

---

## 📚 100 Days of Python

| Day     | Topic                                   |
| ------- | ------------------------------          |
| Day 01  | Introduction to Python                  |
| Day 02  | Applications of Python                  |
| Day 03  | Modules and pip                         |
| Day 04  | Our First Program                       |
| Day 05  | Comments and `print()`                  |
| Day 06  | Variables and Data Types                |
| Day 07  | Exercise 1 — Create a Calculator        |
| Day 08  | Exercise 1 — Solution                   |
| Day 09  | Typecasting                             |
| Day 10  | Taking User Input                       |
| Day 11  | Strings                                 |
| Day 12  | String Slicing                          |
| Day 13  | String Methods                          |
| Day 14  | If-Else Conditionals                    |
| Day 15  | Exercise 2 — Good Morning Sir           |
| Day 16  | Match Case                              |
| Day 17  | For Loops                               |
| Day 18  | While Loops                             |
| Day 19  | `break` and `continue`                  |
| Day 20  | Functions                               |
| Day 21  | Function Arguments                      |
| Day 22  | Introduction to Lists                   |
| Day 23  | List Methods                            |
| Day 24  | Introduction to Tuples                  |
| Day 25  | Operations on Tuples                    |
| Day 26  | Exercise 2 — Solution                   |
| Day 27  | Exercise 3                              |
| Day 28  | f-Strings                               |
| Day 29  | Docstrings                              |
| Day 30  | Recursion                               |
| Day 31  | Sets                                    |
| Day 32  | Set Methods                             |
| Day 33  | Dictionaries                            |
| Day 34  | Dictionary Methods                      |
| Day 35  | For Loop with Else                      |
| Day 36  | Exception Handling                      |
| Day 37  | `finally` Keyword                       |
| Day 38  | Custom-Errors                           |
| Day 39  | Exercise-3-Solution                     |
| Day 40  | Exercise-4                              |
| Day 41  | Short-Hand-if-else                      |
| Day 42  | Enumerate                               |
| Day 43  | Virtual Environment                     |
| Day 44  | How `import` works                      |
| Day 45  | if-name-main-in-Python                  |
| Day 46  | os-Module                               |
| Day 47  | Exercise 4 - Solution                   |
| Day 48  | local-vs-global-variables               |
| Day 49  | File-IO                                 |
| Day 50  | read-readlines-and-other-methods        |
| Day 51  | seek-and-tell-functions                 |
| Day 52  | Lambda Functions                        |
| Day 53  | Map-Filter-and-Reduce                   |
| Day 54  | —                                       |
| Day 55  | —                                       |
| Day 56  | —                                       |
| Day 57  | —                                       |
| Day 58  | —                                       |
| Day 59  | —                                       |
| Day 60  | —                                       |
| Day 61  | —                                       |
| Day 62  | —                                       |
| Day 63  | —                                       |
| Day 64  | —                                       |
| Day 65  | —                                       |
| Day 66  | —                                       |
| Day 67  | —                                       |
| Day 68  | —                                       |
| Day 69  | —                                       |
| Day 70  | —                                       |
| Day 71  | —                                       |
| Day 72  | —                                       |
| Day 73  | —                                       |
| Day 74  | —                                       |
| Day 75  | —                                       |
| Day 76  | —                                       |
| Day 77  | —                                       |
| Day 78  | —                                       |
| Day 79  | —                                       |
| Day 80  | —                                       |
| Day 81  | —                                       |
| Day 82  | —                                       |
| Day 83  | —                                       |
| Day 84  | —                                       |
| Day 85  | —                                       |
| Day 86  | —                                       |
| Day 87  | —                                       |
| Day 88  | —                                       |
| Day 89  | —                                       |
| Day 90  | —                                       |
| Day 91  | —                                       |
| Day 92  | —                                       |
| Day 93  | —                                       |
| Day 94  | —                                       |
| Day 95  | —                                       |
| Day 96  | —                                       |
| Day 97  | —                                       |
| Day 98  | —                                       |
| Day 99  | —                                       |
| Day 100 | —                                       |

---

## 🗂️ Repository Structure

```text
100-Days-of-Python/
│
├── 01-Day01-Introduction-to-Python/
├── 02-Day02-Applications-of-Python/
├── 03-Day03-Modules-and-Pip/
├── 04-Day04-Our-First-Program/
├── 05-Day05-Comments-and-Print/
├── 06-Day06-Variables-and-Datatypes/
├── 07-Day07-Exercise-1-Create-a-Calculator/
├── 08-Day08-Exercise-1-Create-a-Calculator-Solution/
├── 09-Day09-Typecasting-in-Python/
├── 10-Day10-Taking-User-Input/
├── 11-Day11-Strings/
├── 12-Day12-Strings-Slicing/
├── 13-Day13-String-Methods/
├── 14-Day14-If-Else-Conditionals/
├── 15-Day15-Exercise-2-Good-Morning-Sir/
├── 16-Day16-Match-Case/
├── 17-Day17-For-Loops/
├── 18-Day18-While-Loops/
├── 19-Day19-break-and-continue/
├── 20-Day20-Functions/
├── 21-Day21-Function-Arguments/
├── 22-Day22-Introduction-to-Lists/
├── 23-Day23-Lists-Methods/
├── 24-Day24-Introduction-to-Tuples/
├── 25-Day25-Operations-on-Tuples/
├── 26-Day26-Exercise-2-Solution/
├── 27-Day27-Exercise-3/
├── 28-Day28-f-strings/
├── 29-Day29-Docstrings/
├── 30-Day30-Recursion/
├── 31-Day31-Sets/
├── 32-Day32-Set-Methods/
├── 33-Day33-Dictionary/
├── 34-Day34-Dictionary-Methods/
├── 35-Day35-For-loop-with-else/
├── 36-Day36-Exception-Handling/
├── 37-Day37-Finally-keyword/
├── 38-Day38-Custom-Errors/
├── 39-Day39-Exercise-3-Solution/
├── 40-Day40-Exercise-4/
├── 41-Day41-Short-Hand-if-else/
├── 42-Day42-Enumerate/
├── 43-Day43-Virtual-Environment/
├── 44-Day44-How-import-works/
├── 45-Day45-if-name-main-in-Python/
├── 46-Day46-os-Module/
├── 47-Day47-Exercise-3-Solution/
├── 48-Day48-local-vs-global-variables/
├── 49-Day49-File-IO/
├── 50-Day50-read-readlines-and-other-methods/
├── 51-Day51-seek-and-tell-functions/
├── 52-Day52-Lambda-Functions/
├── 53-Day53-Map-Filter-and-Reduce/
├── 54-Day54/
├── 55-Day55/
├── 56-Day56/
├── 57-Day57/
├── 58-Day58/
├── 59-Day59/
├── 60-Day60/
├── 61-Day61/
├── 62-Day62/
├── 63-Day63/
├── 64-Day64/
├── 65-Day65/
├── 66-Day66/
├── 67-Day67/
├── 68-Day68/
├── 69-Day69/
├── 70-Day70/
├── 71-Day71/
├── 72-Day72/
├── 73-Day73/
├── 74-Day74/
├── 75-Day75/
├── 76-Day76/
├── 77-Day77/
├── 78-Day78/
├── 79-Day79/
├── 80-Day80/
├── 81-Day81/
├── 82-Day82/
├── 83-Day83/
├── 84-Day84/
├── 85-Day85/
├── 86-Day86/
├── 87-Day87/
├── 88-Day88/
├── 89-Day89/
├── 90-Day90/
├── 91-Day91/
├── 92-Day92/
├── 93-Day93/
├── 94-Day94/
├── 95-Day95/
├── 96-Day96/
├── 97-Day97/
├── 98-Day98/
├── 99-Day99/
├── 100-Day100/
│
└── README.md
```

---

## 📈 Progress

### Current Progress: **53 / 100 Days**

```text
████████████████████▊░░░░░░░░░░░░░░░░░░ 53%
```

The progress section will be updated throughout the challenge.

---

## 💻 What This Repository Contains

Each day represents one step in the learning journey.

The repository will contain:

* 📝 Concepts learned
* 💻 Python code
* 🧩 Exercises
* 🧠 Practice problems
* 🔨 Small programs
* 🚀 Projects and practical implementations

The topics will be added to the README as the journey progresses.

---

## 🧭 Learning Approach

The goal is not simply to complete 100 days.

For each topic, the approach is:

**Learn → Understand → Code → Practice → Solve → Commit**

Every day's work is uploaded to this repository to maintain a record of the learning journey and progress.

---

## 🚀 The Goal

By the end of these **100 Days of Python**, the aim is to have a solid understanding of Python fundamentals and a strong base for moving into:

**Advanced Python → Python Libraries → Automation → Data Analytics → AI/ML → Data Science → GenAI & Agentic AI**

---

## 🏁 100 Days. One Journey.

This repository is a record of my progress from **Day 01 to Day 100**.

No skipping the journey.

No waiting for perfection.

Just **learning, coding, and improving every day. 🐍🔥**

> **Consistency over perfection.**
